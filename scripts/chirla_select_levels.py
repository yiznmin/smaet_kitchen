...
## 判定
  ...
  L2    單鏡頭、獨佔(**連消失期間也不能有別人**)、≥2 個區段,
        ...
        與 >5 秒(M4 救不回,只能靠 M5)
  L3    2~3 台鏡頭、≥1 次真實轉場(時間不重疊、Δt ≤ 30 秒)
        ⚠ 獨佔在這裡是**屬性不是條件** —— CHIRLA 全場只有一個人的時刻只有 1 個,
          硬性要求會挑不到任何窗。L3A(獨佔)拍影片、L3B(非獨佔)算指標,
          因為誤併率需要場上有 ≥2 個身份才存在。
...
from eval_m4m5_chirla import load_gt, phys                      # noqa: E402
...
def make_window(level, rule, split, seq, cams, gid, lo, hi, stride, occ, **extra):
    ...
    return dict(
        window_id=f"{level}-{split}-{seq}-{'+'.join(cams)}-{lo - 1:06d}",
...
def find_l1(seq, split, pres, occ, cams_all, p):
    out = []
    for (gid, cam), frames in pres.items():
        ...
        out.append(make_window("L1", "L1", split, seq, [cam], gid, s, hi,
                               ...
                               gaps_s=[], gap_band=None))
    return out


def find_l2(seq, split, pres, occ, cams_all, p):
    out = []
    for (gid, cam), frames in pres.items():
        segs = segments(frames, p["merge_gap"])
        if len(segs) < 2:
        ...
        for (s1, e1), (s2, e2) in zip(segs, segs[1:]):
            ...
            if min((e1 - s1), (e2 - s2)) / FPS < p["l2_min_seg_s"]:
                continue
            if not exclusive(occ, [cam], gid, s1, e2, p["stride"]):
                continue
            band = "within_buffer" if gap <= p["l2_buffer_s"] else "beyond_buffer"
            out.append(make_window("L2", "L2", split, seq, [cam], gid, s1, e2,
                                   p["stride"], occ,
                                   segments_s=[round((e1 - s1) / FPS, 3),
                                               round((e2 - s2) / FPS, 3)],
                                   gaps_s=[round(gap, 3)], gap_band=band))
    return out


def find_l3(seq, split, pres, occ, cams_all, p, overlapping):
    """同一身份跨鏡頭:取相鄰兩段分屬不同鏡頭、時間不重疊、Δt 在窗內。"""
    by_id = defaultdict(list)
    for (gid, cam), frames in pres.items():
        for s, e in segments(frames, p["merge_gap"]):
            by_id[gid].append((s, e, cam))
    out = []
    for gid, segs in by_id.items():
        segs.sort()
        for (s1, e1, c1), (s2, e2, c2) in zip(segs, segs[1:]):
            ...
            if not (0 < dt <= p["l3_max_dt_s"]):
                continue
            cams = [c1, c2]
            rule = "L3A" if exclusive(occ, cams, gid, s1, e2, p["stride"]) else "L3B"
            out.append(make_window("L3", rule, split, seq, cams, gid, s1, e2,
                                   p["stride"], occ, segments_s=[], gaps_s=[round(dt, 3)],
                                   gap_band=None,
                                   transitions=[dict(**{"from": c1}, to=c2,
                                                     dt_s=round(dt, 3),
...
def pick(windows, take):
    """固定排序鍵取前 N,且同一 (seq, camera) 最多一個。

    ⚠ 排序鍵**不含任何品質指標** —— 用結果好壞挑要拍的片段,等於用測試集選樣本。
    ⚠ 同一 (seq, camera) 限一個:否則四段影片可能全來自同一分鐘的畫面。
    """
    seen, chosen = set(), []
    for w in sorted(windows, key=lambda x: (x["level"], x["split"], x["seq"],
...
def selfcheck(windows, splits, p):
    ...
    for w in windows:
        ...
        if w["level"] == "L1" and (len(w["gaps_s"]) or len(w["segments_s"]) != 1):
            bad.append(f"{w['window_id']}:L1 不該有間隔")
        if w["level"] == "L2" and (len(w["gaps_s"]) != 1 or len(w["segments_s"]) != 2):
            bad.append(f"{w['window_id']}:L2 必須剛好兩段一隔")
        if w["level"] == "L2" and not w["exclusive"]:
            bad.append(f"{w['window_id']}:L2 必須獨佔")
        if w["level"] == "L3" and len(w["cameras"]) < 2:
            bad.append(f"{w['window_id']}:L3 少於 2 台鏡頭")
...
def main():
    ...
    ap.add_argument("--l3-max-dt-s", type=float, default=30.0)
    ap.add_argument("--take", type=int, default=2, help="每個 (level, split) 要拍幾段影片")
    ap.add_argument("--topology", default=str(ROOT / "configs" / "camera_topology.chirla.yaml"))
    ap.add_argument("--train-seqs", nargs="+", required=True)
    ap.add_argument("--test-seqs", nargs="+", required=True)
    ...
    p = dict(stride=args.stride, merge_gap=args.merge_gap,
             ...
             l3_max_dt_s=args.l3_max_dt_s, take=args.take)
    ...
    for split, seqs in splits.items():
        for seq in seqs:
            ...
            cams = sorted(gt)
            found = (find_l1(seq, split, pres, occ, cams, p)
                     + find_l2(seq, split, pres, occ, cams, p)
                     + find_l3(seq, split, pres, occ, cams, p, overlapping))
            for w in found:
                stage[f"{w['level']}|{split}"] += 1
            windows += found
            print(f"  {seq:<10}({split:<5}) 身份 {len({g for g, _ in pres})} 個 → 窗 {len(found)}")

    for level in ("L1", "L2", "L3"):
        for split in splits:
            pool = [w for w in windows if w["level"] == level and w["split"] == split]
            if level == "L3":          # 拍片優先用獨佔的那種(字面符合「單人跨鏡頭」)
                pool = [w for w in pool if w["selection_rule"] == "L3A"] or pool
    ...
    summary = Counter(f"{w['level']}|{w['split']}" for w in windows)
    ...
    print("\n每級產量(⚠ 0 就是 0,不從另一側補):")
    for level in ("L1", "L2", "L3"):
        row = "  ".join(f"{s}={summary.get(f'{level}|{s}', 0)}" for s in splits)
        print(f"  {level}  {row}")
