...
# 要給業主與教授看的東西。畫面上同一位 chef_id 在**所有鏡頭裡同一個顏色**,
# 這樣「跨鏡頭認得是同一個人」這件事才看得出來,不然一堆數字沒人看得懂。
#
# 2026-09-16:實作搬到 src/common/draw_tracks.py。難度分級交付要一支離線渲染器
# (重放 tracks.csv 產結果影片),它與這裡必須畫出完全一樣的畫面 —— 兩份實作
# 遲早會漂開,而漂開時影片與指標對不上,沒有人會發現。搬動時邏輯一個字沒改。
from common.draw_tracks import chef_color, draw_panel, stitch   # noqa: E402,F401
