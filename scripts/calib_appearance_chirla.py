...
CHIRLA 反而更貼近 M5 的部署情境:7 台分佈在不同房間的鏡頭、橫跨 7 個月、同一人會換衣服。
所以這裡直接從已匯出的 embedding 量跨相機的 cosine 分布。

⚠ 論文寫 CHIRLA 是「非重疊」,**實測不成立**(cam2/cam3 拍同一個房間,
  40% 的觀測被 2 台以上同時看到,見 docs/CHIRLA_鏡頭佈局實測_20260903.md)。
  這對本腳本的影響:量出來的「跨相機」分布**混合了重疊與非重疊兩種路徑**,
  比純轉場路徑樂觀。要分開量的話得先照相機對分組,本輪沒做。
